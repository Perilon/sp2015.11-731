...
def simple_meteor(hypothesis, reference, alpha, delta_nouns, delta_verbs, delta_other, w_exact, w_stems, w_synonyms):
...
##        print "reference = ", reference, len(reference), "\n"
        ...
        hyp_nouns, ref_nouns, hyp_verbs, ref_verbs, hyp_other, ref_other = [], [], [], [], [], []
...
##        print "Leftover_ref_other = ", Leftover_ref_other
        ...
        hyp_nouns_stems, ref_nouns_stems, hyp_verbs_stems, ref_verbs_stems, hyp_other_stems, ref_other_stems = \
                         [], [], [], [], [], []
...
##        def Precision(delta_nouns, delta_verbs, delta_other, w_exact, w_stems, count_noun_matches, count_verb_matches, count_other_matches, hypothesis):
##                if len(hypothesis) == 0: return 0.0
##                else:
##                        return ((delta_nouns * count_noun_matches) + (delta_verbs * count_verb_matches) + (delta_other * count_other_matches)) / len(hypothesis)
##
##        def Recall(delta_nouns, delta_verbs, delta_other, w_exact, w_stems, count_noun_matches, count_verb_matches, count_other_matches, reference):
##                if len(reference) == 0: return 0.0
##                else:
##                        return ((delta_nouns * count_noun_matches) + (delta_verbs * count_verb_matches) + (delta_other * count_other_matches)) / len(reference)

        def Precision2(delta_nouns, delta_verbs, delta_other, w_exact, w_stems, count_exact_noun_matches, count_exact_verb_matches, count_exact_other_matches, \
                       count_stemmed_noun_matches, count_stemmed_verb_matches, count_stemmed_other_matches, hypothesis):
                if len(hypothesis) == 0: return 0.0
                else:
                        return ((w_exact * ((delta_nouns * count_exact_noun_matches) + (delta_verbs * count_exact_verb_matches) + (delta_other * count_exact_other_matches))) \
                               + (w_stems * ((delta_nouns * count_stemmed_noun_matches) + (delta_verbs * count_stemmed_verb_matches) + (delta_other * count_stemmed_other_matches)))) \
                               / len(hypothesis)

        def Recall2(delta_nouns, delta_verbs, delta_other, w_exact, w_stems, count_exact_noun_matches, count_exact_verb_matches, count_exact_other_matches, \
                       count_stemmed_noun_matches, count_stemmed_verb_matches, count_stemmed_other_matches, reference):
                if len(reference) == 0: return 0.0
                else:
                        return ((w_exact * ((delta_nouns * count_exact_noun_matches) + (delta_verbs * count_exact_verb_matches) + (delta_other * count_exact_other_matches))) \
                               + (w_stems * ((delta_nouns * count_stemmed_noun_matches) + (delta_verbs * count_stemmed_verb_matches) + (delta_other * count_stemmed_other_matches)))) \
        ...
        P2 = Precision2(delta_nouns, delta_verbs, delta_other, w_exact, w_stems, count_exact_noun_matches, count_exact_verb_matches, count_exact_other_matches, \
                       count_stemmed_noun_matches, count_stemmed_verb_matches, count_stemmed_other_matches, hypothesis)
        R2 = Recall2(delta_nouns, delta_verbs, delta_other, w_exact, w_stems, count_exact_noun_matches, count_exact_verb_matches, count_exact_other_matches, \
                       count_stemmed_noun_matches, count_stemmed_verb_matches, count_stemmed_other_matches, reference)
...
def main():
        ...
        parser.add_argument('-dn', '--delta_nouns', default=.4, type=float, help='noun weight parameter')
        parser.add_argument('-dv', '--delta_verbs', default=.4, type=float, help='verb weight parameter')
        parser.add_argument('-do', '--delta_other', default=.2, type=float, help='other weight parameter')
        ...
        for h1, h2, reference in islice(sentences(), opts.num_sentences):
                h1_val = simple_meteor(h1, reference, opts.alpha, opts.delta_nouns, opts.delta_verbs, opts.delta_other, opts.w_exact, opts.w_stems, opts.w_synonyms)
                h2_val = simple_meteor(h2, reference, opts.alpha, opts.delta_nouns, opts.delta_verbs, opts.delta_other, opts.w_exact, opts.w_stems, opts.w_synonyms)
