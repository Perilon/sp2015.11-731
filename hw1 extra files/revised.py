...
##        print "matches.values() = ", matches.values(), "\n"
        ...
        for (english_word_index, english_word) in enumerate(pair[1]):
            ...
            print str(pair[0].index(german_choice)) + "-" + str(english_word_index),
        print "\n"
    ...
    for(g,e) in bitext:
        for (e_idx, target_word_e) in enumerate(e):
            ...
            for (g_idx, source_word_g) in enumerate(g):
                if translation_probability[(target_word_e, source_word_g)]>score:
                    best_idx=g_idx
                    score=translation_probability[(target_word_e, source_word_g)]
            print "%i-%i" % (best_idx,e_idx),
        print "\n"
