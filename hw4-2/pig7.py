...
for line in read_input:
	...
	if len(left_context) > 0:
		left_context_set = []
		for item in left_context:
			item = remove_accents(item[:SL])
			left_context_set.append(item)
		left_context_set = sorted(set(left_context_set))
		prev = "".join(left_context_set)
		#prev = left_context[-1]
		#prev = prev[:4]
	else:
		prev = "<s>"
	if len(right_context) > 0:
		right_context_set = []
		for item in right_context:
			item = remove_accents(item[:SL])
			right_context_set.append(item)
		right_context_set = sorted(set(right_context_set))
		Next = "".join(right_context_set)
...
def update_ttable(filename):
	...
	with open(filename) as f:
		for i, line in enumerate(f):
			...
			candidate = "src:" + source[:SL] + "_tgt:" + target[:SL]
